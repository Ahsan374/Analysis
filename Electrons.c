...
vector <string> _Runs = {"1695684864.root", "1695781104.root", "1695624750.root", "1695667820.root", "1695791105.root","1695678689.r>
...
case 2:
        ...
        break;
case 3:
        graph ->AddPoint(3.0, electron_percentage);
        break;
case 4:
        graph ->AddPoint(4.5, electron_percentage);
        break;
default:
        graph ->AddPoint(10.0, electron_percentage);
}
vector_entry_index++;
}
...
TCanvas* canvas = new TCanvas("canvas", "Electron Percentage", 900, 100, 500, 500);
...
graph ->Draw("alp");
latex.DrawLatex(0.5, 0.04, "Fig 1: Percentage of Electrons in T10");
TLegend *legend = new TLegend(0.7, 0.7, 0.8, 0.8);
legend ->AddEntry(graph, "Electron", "ple");
legend ->Draw();
canvas ->Update();
graph ->SaveAs("Electron_Percentage.png");
//program ends here
stopwatch.Stop();
